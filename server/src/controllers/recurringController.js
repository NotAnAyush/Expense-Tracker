...
exports.createRecurringExpense = asyncHandler(async (req, res) => {
  ...
  if (!title || amount === undefined || !category || !nextOccurrence) {
    throw new BadRequestError('Please provide title, amount, category, and next occurrence date');
  }
...
exports.getRecurringHistory = asyncHandler(async (req, res) => {
  const item = await RecurringExpense.findOne({ _id: req.params.id, userId: req.user._id });
  if (!item) {
...
exports.recordRecurringPayment = asyncHandler(async (req, res) => {
  const item = await RecurringExpense.findOne({ _id: req.params.id, userId: req.user._id });
  if (!item) {
    throw new NotFoundError('Subscription not found');
  }
